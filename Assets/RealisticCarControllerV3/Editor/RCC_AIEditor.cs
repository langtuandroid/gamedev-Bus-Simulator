...
[CustomEditor(typeof(RCC_AICarControllerBSR))]
public class RCC_AIEditor : Editor {

	RCC_AICarControllerBSR aiController;
	...
	static void CreateAIBehavior(){
		...
		if(!Selection.activeGameObject.GetComponent<RCC_AICarControllerBSR>() && Selection.activeGameObject.GetComponent<RCC_CarControllerV3>()){
			Selection.activeGameObject.AddComponent<RCC_AICarControllerBSR>();
	...
	static void CreateWaypointsContainer(){
		...
		if(GameObject.FindObjectOfType<RCC_AIWaypointsContainerBSR>() == null){
			...
			wp.AddComponent<RCC_AIWaypointsContainerBSR>();
	...
	static void CreateBrakeZonesContainer(){
		...
		if(GameObject.FindObjectOfType<RCC_AIBrakeZonesContainerBSR>() == null){
			...
			bz.AddComponent<RCC_AIBrakeZonesContainerBSR>();
	...
	public override void OnInspectorGUI () {
		...
		aiController = (RCC_AICarControllerBSR)target;
...
//			aiController.gameObject.GetComponent<RCC_CarControllerV3>().autoReverse = true;
		...
		EditorGUILayout.Separator();
		EditorGUILayout.LabelField("Current Waypoint: ", aiController.currentWaypointBSR.ToString());
		EditorGUILayout.LabelField("Laps: ", aiController.lapBSR.ToString());
		EditorGUILayout.LabelField("Total Waypoints Passed: ", aiController.totalWaypointPassedBSR.ToString());
		EditorGUILayout.LabelField("Ignoring Waypoint Due To Unexpected Obstacle: ", aiController.ignoreWaypointNowBSR.ToString());
		EditorGUILayout.Separator();
