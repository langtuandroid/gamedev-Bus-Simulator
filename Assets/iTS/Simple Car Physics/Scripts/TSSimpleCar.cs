...
public class TSSimpleCar : MonoBehaviour {
	...
	public AudioClip [] hornClips=new AudioClip[0];
...
//	public float mass_center = 0.025f;
	...
	/// </summary>
	public bool superSimplePhysics = true;
...
//				Debug.Log ("Set center of mass for car" + " " +myTransform.root.name);
	...
	void OnTurnRight(bool isTurning)
	{
		if (turnRightLight != null){
			if (isTurning)
				StartCoroutine(LightBlinking(turnRightLight,0.2f));
			else{
				blinking = false;
				turnRightLight.SetActive(false);
			}
	...
	void OnTurnLeft(bool isTurning)
	{
		if (turnLeftLight != null){
			if (isTurning)
				StartCoroutine(LightBlinking(turnLeftLight,0.2f));
			else{
				blinking = false;
				turnLeftLight.SetActive(false);
			}
	...
	IEnumerator LightBlinking(GameObject blinkingLight, float blinkingTiming)
	{
		blinking  = true;
		while(blinking)
		{
			...
			yield return new WaitForSeconds(blinkingTiming);
	...
	void Update () {
		if (!crashed){
			if (superSimplePhysics && !upSideDown)
			{
				...
				GetComponent<AudioSource>().pitch = Mathf.Abs(mySpeed / maxSpeed) + 0.75f ;
...
//				}
				...
				// set the audio pitch to the percentage of RPM to the maximum RPM plus one, this makes the sound play
				// up to twice it's pitch, where it will suddenly drop when it switches gears.
				GetComponent<AudioSource>().pitch = Mathf.Abs(EngineRPM / MaxEngineRPM) + 0.75f ;
				// this line is just to ensure that the pitch does not reach a value higher than is desired.
				if ( GetComponent<AudioSource>().pitch > 2.0f ) {
					GetComponent<AudioSource>().pitch = 2.0f;
				}
	...
	public void BrakeLight(float brakeInput)
	{
		if (enableDisableBrakeLights){
			if (brakeLigths != null){
				if (brake != 0) {
					brakeLigths.SetActive (true);
				} else if (brake == 0) {
					brakeLigths.SetActive (false);
				}
...
//		}
	...
	{
		rotationAmount = Vector3.right * (mySpeed * 1.6f * Time.deltaTime * Mathf.Rad2Deg);
		for (int i = 0; i < frontWheels.Length;i++)
		{
			frontWheels[i].Rotate(rotationAmount);
			Vector3 tempAngles = frontWheels[i].localEulerAngles;
			tempAngles.y = steering;
			//frontWheels[i].localEulerAngles = tempAngles;
		}
	...
	void FixedUpdate()
	{
		if (!crashed && superSimplePhysics && !upSideDown&&addBrake==false)
		{
	...
	void OnTriggerEnter(Collider col){
		if (col.gameObject.tag == "Bus") {
			if (!audioSource.isPlaying) {
				audioSource.clip = hornClips [UnityEngine.Random.Range (0, hornClips.Length)];
				audioSource.Play ();
			}
				addBrake = true;
		}
